...
export default function SecuritySettingsModal({ darkMode, onClose, securityData, onUpdate }) {
  ...
  const getSecurityId = () => {
    return securityData?.employeeId || 
  ...
  const [profileData, setProfileData] = useState({
    ...
    employeeId: getSecurityId(),
    assignedGate: securityData?.assignedGate || securityData?.gate || '',
    role: securityData?.role || 'Security Personnel',
    joinDate: formatDate(securityData?.created_at || securityData?.joinDate || '')
  });
        ...
        <div className="border-b border-white/10 p-4 sm:p-6 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-1.5 sm:p-2 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg sm:rounded-xl shadow-lg">
              <Shield className="w-4 h-4 sm:w-5 sm:h-6 text-white" />
            </div>
            <div>
              <h2 className={`text-lg sm:text-xl font-bold ${textPrimary}`}>Security Information</h2>
                ...
                <div className="text-center sm:text-left flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row items-center sm:items-end gap-3 sm:gap-4">
                    <div className="flex-1">
                      <p className={`text-sm ${textSecondary} mb-1`}>Assigned Gate</p>
                      <p className={`font-semibold ${textPrimary} text-base truncate`}>{profileData.assignedGate || 'Not assigned'}</p>
                    </div>
                    <button
              ...
              <div className="h-full">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 h-full">
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className={`p-4 sm:p-5 rounded-lg sm:rounded-xl ${darkMode ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-blue-50 border border-blue-200'}`}>
                      <div className="flex items-start gap-3">
                        <div className={`p-2 rounded-lg ${darkMode ? 'bg-blue-500/20' : 'bg-blue-100'} flex-shrink-0`}>
                          <Key className={`w-4 h-4 sm:w-5 sm:h-5 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <label className={`block text-xs ${textSecondary} mb-1`}>Employee ID</label>
                          <p className={`font-bold ${textPrimary} text-sm sm:text-base truncate`}>{profileData.employeeId}</p>
                        </div>
                      </div>
                    </div>

                    {/* Email */}
                    {/* Email field with working tooltip */}
